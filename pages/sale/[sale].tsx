...
export default function Sale({ attributes, photos, content, gallery }: { gallery: string, photos: Array<{src: string; alt: string; blurDataURL: string}>, attributes: Record<string, any>, content: string}) {
  ...
  return (
    <div className={styles.container}>
      <Head>
        ...
        <meta property="og:url" content={`${process.env.URL || ''}/sale/${gallery}`} />
...
export async function getStaticProps({ ...ctx }) {
  const { sale } = ctx.params
  const { attributes, body } = await import(`../../content/sales/${sale}.md`)
...
export async function getStaticPaths() {
  const saleSlugs = ((context) => {
    ...
    return data
  })(require.context('../../content/sales', true, /\.md$/))

  const paths = saleSlugs.map((slug: string) => `/sale/${slug}`)
